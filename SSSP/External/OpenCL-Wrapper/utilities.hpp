...
inline float cl_rsqrt(const float x) {
	return 1.0f/sqrt(x);
}
...
inline double cl_rsqrt(const double x) {
	return 1.0/sqrt(x);
}
...
inline int cl_min(const int x, const int y) {
	return x<y?x:y;
}
inline int cl_max(const int x, const int y) {
	return x>y?x:y;
}
inline int clamp(const int x, const int a, const int b) {
	return cl_min(cl_max(x, a), b);
}
...
inline uint cl_min(const uint x, const uint y) {
	return x<y?x:y;
}
inline uint cl_max(const uint x, const uint y) {
	return x>y?x:y;
}
...
inline slong cl_min(const slong x, const slong y) {
	return x<y?x:y;
}
inline slong cl_max(const slong x, const slong y) {
	return x>y?x:y;
}
...
inline ulong cl_min(const ulong x, const ulong y) {
	return x<y?x:y;
}
inline ulong cl_max(const ulong x, const ulong y) {
	return x>y?x:y;
}
...
inline string to_string(float x, const uint decimals) { // convert float to string with specified number of decimals
	...
	const float power = pow(10.0f, cl_min(decimals, 8u));
	x += 0.5f/power; // rounding
	const ulong integral = (ulong)x;
	const uint decimal = (uint)((x-(float)integral)*power);
	return s+to_string(integral)+(decimals==0u ? "" : "."+decimal_to_string_float(decimal, cl_min((int)decimals, 8)));
}
inline string to_string(double x, const uint decimals) { // convert float to string with specified number of decimals
	...
	const double power = pow(10.0, cl_min(decimals, 16u));
	x += 0.5/power; // rounding
	const ulong integral = (ulong)x;
	const ulong decimal = (ulong)((x-(double)integral)*power);
	return s+to_string(integral)+(decimals==0u ? "" : "."+decimal_to_string_double(decimal, cl_min((int)decimals, 16)));
}
...
inline string substring(const string& s, const uint start, uint length=max_uint) {
	return s.substr(start, cl_min(length, (uint)s.length()-start));
}
...
inline string alignl(const uint n, const string& x="") { // converts x to string with spaces behind such that length is n if x is not longer than n
	...
	return s.substr(0, cl_max(n, (uint)x.length()));
}
inline string alignr(const uint n, const string& x="") { // converts x to string with spaces in front such that length is n if x is not longer than n
	...
	for(uint i=0u; i<n; i++) s += " ";
	s += x;
	return s.substr((uint)cl_min((int)s.length()-(int)n, (int)n), s.length());
}
